...
class RolesTest extends TestCase
{
    ...
    public function testRoles()
    {
        ...
        // Adding a New Role
        $this->visit(route('omega.roles.index'))->click('Add New')->seePageIs(route('omega.roles.create'));
        $this->type('superadmin', 'name');
        $this->type('Super Admin', 'display_name');
        $this->press('Submit');
        $this->seePageIs(route('omega.roles.index'));
        $this->seeInDatabase('roles', ['name' => 'superadmin']);

        // Editing a Role
        $this->visit(route('omega.roles.edit', 2));
        ...
        // Deleting a Role
        $response = $this->call('DELETE', route('omega.roles.destroy', $superadmin_role->id), ['_token' => csrf_token()]);
