...
class UserProfileTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->editPageForTheCurrentUser = route('omega.users.edit', ['user' => $this->user->id]);

        $this->listOfUsers = route('omega.users.index');
    }

    public function testCanSeeTheUserInfoOnHisProfilePage()
    {
        $this->visit(route('omega.profile'))
    ...
    public function testCanEditUserPassword()
    {
        $this->visit(route('omega.profile'))
             ...
             ->see('Edit User')
             ->seePageIs($this->editPageForTheCurrentUser)
             ->type('omega-rocks', 'password')
             ->press('Submit')
             ->seePageIs($this->listOfUsers);

        $updatedPassword = DB::table('users')->where('id', 1)->first()->password;
        $this->assertTrue(Hash::check('omega-rocks', $updatedPassword));
    }
