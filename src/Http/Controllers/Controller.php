...
use artworx\omegacp\Traits\AlertsMessages;
...
abstract class Controller extends BaseController
{
        ...
        switch ($row->type) {
            ...
            case 'file':
                if ($file = $request->file($row->field)) {
                    ...
                    $request->file($row->field)->storeAs(
                        ...
                        config('omega.storage.disk', 'public')
                    );
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    Storage::disk(config('omega.storage.disk'))->put($fullPath, (string) $image, 'public');
                    ...
                    if (isset($options->thumbnails)) {
                        foreach ($options->thumbnails as $thumbnails) {
                            ...
                            Storage::disk(config('omega.storage.disk'))->put($path.$filename.'-'.$thumbnails->name.'.'.$file->getClientOriginalExtension(),
                                (string) $image, 'public'
                            );
                        }
                    }
    ...
    {
        if (Storage::disk(config('omega.storage.disk'))->exists($path)) {
            Storage::disk(config('omega.storage.disk'))->delete($path);
        }
    }
}
