...
@section('css')
    <script type="text/javascript" src="{{ config('omega.assets_path') }}/js/vue21.min.js"></script>
    <link rel="stylesheet" href="{{ config('omega.assets_path') }}/css/database.css">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="omega-data"></i> Database
        <a href="{{ route('omega.database.create') }}" class="btn btn-success"><i class="omega-plus"></i>
            Create New Table</a>
    </h1>
@stop
    ...
    <div class="page-content container-fluid">
        ...
        <div class="row">
            ...
            @foreach($tables as $table)
                    @continue(in_array($table->name, config('omega.database.tables.hidden', [])))
                    <tr>
                        <td>
                            <p class="name">
                                <a href="{{ route('omega.database.show', $table->name) }}"
                        ...
                        <td>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('omega.database.edit_bread', $table->name) }}"
                            ...
                            @else
                                <a href="{{ route('omega.database.create_bread', ['name' => $table->name]) }}"
                                   class="btn-sm btn-default">
                                    <i class="omega-plus"></i> Add BREAD to this table
                                </a>
                            @endif
                            </div>
                        </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="omega-trash"></i> Are you sure you want to delete the BREAD for
                        the <span id="delete_builder_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('omega.database.delete_bread', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="omega-trash"></i> Are you sure you want to delete the <span
                                id="delete_table_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('omega.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="omega-data"></i> @{{ table.name }}</h4>
                </div>
