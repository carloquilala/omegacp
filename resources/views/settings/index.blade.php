...
@section('css')
    <style>
        .panel-actions .omega-trash {
            cursor: pointer;
        }

        .panel-actions .omega-trash:hover {
            color: #e94542;
        }
...
@section('head')
    <script type="text/javascript" src="{{ config('omega.assets_path') }}/lib/js/jsonarea/jsonarea.min.js"></script>
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="omega-settings"></i> Settings
    </h1>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="alert alert-info">
            ...
            <p>You can get the value of each setting anywhere on your site by calling <code>Omega::setting('key')</code></p>
        </div>
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('omega.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Omega::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('omega.settings.move_up', $setting->id) }}">
                                <i class="sort-icons omega-sort-asc"></i>
                            </a>
                            <a href="{{ route('omega.settings.move_down', $setting->id) }}">
                                <i class="sort-icons omega-sort-desc"></i>
                            </a>
                            <i class="omega-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('omega.storage.disk'))->exists($setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('omega.settings.delete_value', $setting->id) }}" class="omega-x"></a>
                                    <img src="{{ Storage::disk(config('omega.storage.disk'))->url($setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="omega-plus"></i> New Setting</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('omega.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="omega-double-down"></i> OPTIONS</a>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .omega-double-down').length) {
                                    $('#toggle_options .omega-double-down').removeClass('omega-double-down').addClass('omega-double-up');
                                } else {
                                    $('#toggle_options .omega-double-up').removeClass('omega-double-up').addClass('omega-double-down');
                                }
                            });
                        });
                    </script>
                    ...
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="omega-plus"></i> Add New Setting
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="omega-trash"></i> Are you sure you want to delete the <span id="delete_setting_title"></span> Setting?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('omega.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('omega.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
        <input name="image" id="upload_file" type="file" onchange="$('#my_form').submit();this.value='';">
        <input type="hidden" name="type_slug" id="type_slug" value="settings">
    </form>

    <script src="{{ config('omega.assets_path') }}/lib/js/tinymce/tinymce.min.js"></script>
    <script src="{{ config('omega.assets_path') }}/js/omega_tinymce.js"></script>
@stop
