...
@section('page_header')
    <h1 class="page-title">
        ...
        @if (Omega::can('add_'.$dataType->name))
            <a href="{{ route('omega.'.$dataType->slug.'.create') }}" class="btn btn-success">
                <i class="omega-plus"></i> Add New
            </a>
        @endif
    </h1>
@stop
                                ...
                                <tr>
                                    @foreach($dataType->browseRows as $row)
                                        <td>
                                            ...
                                            @if($row->type == 'image')
                                                <img src="@if( strpos($data->{$row->field}, 'http://') === false && strpos($data->{$row->field}, 'https://') === false){{ Omega::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif" style="width:100px">
                                    ...
                                    <td class="no-sort no-click" id="bread-actions">
                                        @if (Omega::can('delete_'.$dataType->name))
                                            <a href="javascript:;" title="Delete" class="btn btn-sm btn-danger pull-right delete" data-id="{{ $data->id }}" id="delete-{{ $data->id }}">
                                                <i class="omega-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
                                            </a>
                                        @endif
                                        @if (Omega::can('edit_'.$dataType->name))
                                            <a href="{{ route('omega.'.$dataType->slug.'.edit', $data->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="omega-edit"></i> <span class="hidden-xs hidden-sm">Edit</span>
                                            </a>
                                        @endif
                                        @if (Omega::can('read_'.$dataType->name))
                                            <a href="{{ route('omega.'.$dataType->slug.'.show', $data->id) }}" title="View" class="btn btn-sm btn-warning pull-right">
                                                <i class="omega-eye"></i> <span class="hidden-xs hidden-sm">View</span>
                                            </a>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="omega-trash"></i> Are you sure you want to delete
                        this {{ strtolower($dataType->display_name_singular) }}?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('omega.'.$dataType->slug.'.index') }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
<link rel="stylesheet" href="{{ config('omega.assets_path') }}/lib/css/responsive.dataTables.min.css">
@endif
@stop
...
@section('javascript')
    <!-- DataTables -->    
    @if(!$dataType->server_side && config('dashboard.data_tables.responsive'))
    <script src="{{ config('omega.assets_path') }}/lib/js/dataTables.responsive.min.js"></script>
    @endif
    <script>
