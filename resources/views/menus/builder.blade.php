...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ config('omega.assets_path') }}/css/nestable.css">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="omega-list"></i>Menu Builder ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="omega-plus"></i> New Menu Item</div>
    </h1>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="omega-trash"></i> Are you sure you want to delete this menu
                        item?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('omega.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}" id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="omega-plus"></i> Create a New Menu Item</h4>
                </div>
                <form action="{{ route('omega.menus.item.add', ['menu' => $menu->id]) }}" id="delete_form" method="POST">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="omega-edit"></i> Edit Menu Item</h4>
                </div>
                <form action="{{ route('omega.menus.item.update', ['menu' => $menu->id]) }}" id="edit_form" method="POST">
                    {{ method_field("PUT") }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <label for="name">Title of the Menu Item</label>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ config('omega.assets_path') }}/js/jquery.nestable.js"></script>
    <script>
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('omega.menus.order',['menu' => $menu->id]) }}', {
